...
def Evaluation(Eseed,lseed,TF):
    ...
    for st in range(save_table_step,num_step+1,save_table_step):
        if TF == True:
            q_tableE = agentE.load_modelsT(lseed,st)
        else:
            q_tableE = agentE.load_modelsF(lseed,st)
        sumR = 0.0
...
def percentileplot(ndarray,ndarrayN):
    ...
    t = np.linspace(0, num_plot, num_plot)
    ...
    c2 = "blue"      # 各プロットの色
    l2 = "Q-learning with EXreplay"   # 各ラベル
    c2N = "red"      # 各プロットの色
    l2N = "Q-learning"   # 各ラベル   
    ax.set_xlabel('step/400')  # x軸ラベル
    ax.set_ylabel('accumulated reward')  # y軸ラベル
    ax.grid()            # 罫線
    plt.xlim([0,num_step/400])
    plt.ylim([-2000,0])  
    ...
    pname = "QL.se"+str(seed)+".st"+str(num_step)+".png"
