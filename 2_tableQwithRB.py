...
class QAgent(Agent):
    ...
    def select_action(self,observation):#評価時はこっち
    ...
    def train(self,action,observation,next_observation,reward):# Qテーブルの更新
        alpha = 3 * pow(10,-4)
        gamma = 0.99
...
class RBufferQ(ReplayBuffer):
    ...
    def sample(self,batch_size,step_count):
        #0からstepcount以下でバッチサイズ個乱数を作って対応する要素を代入
...
step_count = 0
q_table = createQtable(10,9)#(1000,9)
...
for e in tqdm(range(num_episode)):
    ...
    for s in range(200):#not done
        action = agent.select_exploratory_action(observation)
        ...
        if step_count >= batch_size:
            ...
            for i in range(batch_size):
                ...
                agent.train(action_l,observation_l,next_observation_l,reward_l)  
    if (e+1) % 2 == 0:
